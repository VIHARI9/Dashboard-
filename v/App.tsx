              ...
              >
                ...
                >
                  ...
                  <div className="tableWrap" style={{ flex: 1 }}>
                    <table className="productionMatrix" style={{width:"100%",minWidth:940,height:"100%",tableLayout:"fixed"}}><thead><tr><th rowSpan={2}>Grade / Type</th><th colSpan={2}>On Date</th><th colSpan={2}>{overview.period.label}</th><th colSpan={2}>YTD</th></tr><tr><th>Cells</th><th>MW</th><th>Cells</th><th>MW</th><th>Cells</th><th>MW</th></tr></thead><tbody><tr className="group"><th colSpan={7}>Production</th></tr>{overview.distribution.map(row=><tr key={row.label} className={row.label === "Good Cells" ? "total" : ""}><th>{row.label}</th><td>{fmt(row.on_date_cells,0)}</td><td>{fmt(row.on_date_mw,2)}</td><td>{fmt(row.period_cells,0)}</td><td>{fmt(row.period_mw,2)}</td><td>{fmt(row.ytd_cells,0)}</td><td>{fmt(row.ytd_mw,2)}</td></tr>)}<tr className="group rejectionGroup"><th colSpan={7}>Rejection</th></tr>{overview.rejection.map(row=><tr key={row.label} className={row.label === "Bad Cells" ? "lossTotal" : ""}><th>{row.label}</th><td>{fmt(row.on_date_cells,0)}</td><td>{fmt(row.on_date_mw,2)}</td><td>{fmt(row.period_cells,0)}</td><td>{fmt(row.period_mw,2)}</td><td>{fmt(row.ytd_cells,0)}</td><td>{fmt(row.ytd_mw,2)}</td></tr>)}</tbody></table>
                  </div>
                </section>
                    ...
                    <table style={{ width: "100%", minWidth: 500, height: "100%", tableLayout: "fixed" }}>
                      <thead>
                        <tr>
                          ...
                          <th style={{ width: "22%", fontSize: 13, padding: "14px 12px" }}>{overview.period.label}</th>
                      ...
                      <tbody>
                        ...
                        {(overview.yield_table ?? []).map((row) => (
                          <tr key={row.label} className={row.label === "Good Cells" ? "total" : ""}>
                            ...
                            <td style={{ fontSize: 14, fontWeight: 650, padding: "14px 12px" }}>{fmt(row.on_date, 2)}%</td>
                            <td style={{ fontSize: 14, fontWeight: 650, padding: "14px 12px" }}>{fmt(row.period, 2)}%</td>
                            <td style={{ fontSize: 14, fontWeight: 650, padding: "14px 12px" }}>{fmt(row.ytd, 2)}%</td>
                          </tr>
                        ))}
                        <tr className="group rejectionGroup"><th colSpan={4} style={{ fontSize: 12, padding: "11px 12px" }}>Rejection</th></tr>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <Line type="stepAfter" dataKey="breakage_target_pct" name="Breakage Target %" stroke="#7B8794" strokeWidth={2.5} strokeDasharray="7 5" dot={false} connectNulls />
                    <Line type="stepAfter" dataKey="er_target_pct" name="ER Target %" stroke="#7B8794" strokeWidth={2.5} strokeDasharray="7 5" dot={false} connectNulls />
                    <Line type="stepAfter" dataKey="or_target_pct" name="OR Target %" stroke="#7B8794" strokeWidth={2.5} strokeDasharray="7 5" dot={false} connectNulls />
                    <Line
                      type="monotone"
