...
def overview(
    ...
    ytd_plan_mw = prorated_target(plan, fy_start, as_of)
    ...
    current_month_achievement_values = (
        ...
        else pd.Series(0.0, index=current_month_data.index)
    )
    ...
    return {
        ...
        "kpis": [
            ...
            comparison(
                ...
                "MW",
                spark_records(production_to_date_data, production_to_date_cumulative),
            ),
            comparison("YTD production", ytd_actual_mw / 1000, None if ytd_plan_mw is None else ytd_plan_mw / 1000, "GW", spark_records(ytd_sorted, ytd_cumulative / 1000)),
            comparison("Run rate", run_rate, required, "MW/day", spark_records(selected_sorted, run_rate_curve)),
            comparison(
                "Plan achievement Current month",
                current_month_actual,
                current_month_full_plan,
                "MW",
                spark_records(current_month_data, current_month_achievement_values),
            ),
        ],
