...
DATA_DIR = Path(r"C:\Users\vijaya.kalyani\Downloads\Dashboard\v2\database")
...
COMPANY_LOGO = APP_DIR / "company_logo.png"
...
def application_header(as_of_date):
    logo_column, title_column, date_column, refresh_column = st.columns(
        [0.85, 4.1, 1.2, 1.35],
    ...
    with logo_column:
        if COMPANY_LOGO.exists():
            st.image(str(COMPANY_LOGO), use_container_width=True)

    with title_column:
        st.markdown(
            '<div class="app-title">SOLAR MANUFACTURING DASHBOARD</div>',
    ...
    with refresh_column:
        refresh_clicked = st.button(
            "↻ Refresh SAP Data",
            key="main_sap_refresh",
            type="primary",
            use_container_width=True,
            help=(
                "Export fresh operational reports from SAP "
...
def plot(fig):
    # Enforce readable black text on every Plotly visual in the light theme.
    fig.update_layout(
    ...
    for annotation in fig.layout.annotations or []:
        annotation.font = dict(color="#111111")
...
# -----------------------------------------------------------------------------
if COMPANY_LOGO.exists():
    st.sidebar.image(str(COMPANY_LOGO), use_container_width=True)
else:
    st.sidebar.markdown("## ReNew Solar Cell MES")
st.sidebar.caption("Data sources, SAP refresh and diagnostics")
st.sidebar.divider()
...
refresh_clicked = application_header(as_of)

if refresh_clicked:
    with st.spinner(
        "Exporting fresh reports from SAP. "
        "Do not use SAP or Excel until this finishes..."
    ):
    ...
    if refresh_success:
        ...
        st.error("SAP refresh failed.")
        with st.expander("View SAP refresh error", expanded=True):
            st.code(refresh_message, language=None)

if "sap_refresh_success" in st.session_state:
    st.success(st.session_state.pop("sap_refresh_success"))
    refresh_details = st.session_state.pop(
        "sap_refresh_details",
        None,
    )
    if refresh_details:
        with st.expander("SAP refresh details", expanded=False):
            st.text(refresh_details)

page = st.segmented_control(
    "Dashboard section",
    options=["Overview", "Production", "Analytics"],
    default="Overview",
...
if page == "Overview":
    ...
    with cells_column:
        ...
        plot(donut(mtd, ["a_cells", "b_cells", "bel_cells", "eb_cells"], ["A Grade", "B Grade", "B-EL Grade", "EB Grade"], "Cells"))
    with mw_column:
        st.subheader("Grade-wise Production — MW")
        plot(donut(mtd, ["a_mw", "b_mw", "bel_mw", "eb_mw"], ["A Grade", "B Grade", "B-EL Grade", "EB Grade"], "MW"))
...
# -----------------------------------------------------------------------------
    ...
    yield_figure = lines(
        analytics_data,
        [("a_yield_pct", "A Grade", COL["green"]), ("b_yield_pct", "B Grade", COL["blue"]), ("bel_yield_pct", "B-EL Grade", COL["orange"]), ("eb_yield_pct", "EB Grade", COL["purple"])],
        "Yield (%)", grain=view_grain(view),
    )
    ...
    rejection_figure = lines(
        analytics_data,
        [("er_pct", "ER %", COL["gray"]), ("for_pct", "FOR %", COL["blue"]), ("breakage_pct", "Breakage %", COL["orange"])],
        "Rejection / Breakage (%)", grain=view_grain(view),
    )
