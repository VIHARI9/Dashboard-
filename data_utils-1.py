...
PLAN_SCHEMA = [
    ("period",      "Month",              [r"^month$", r"^date$"]),
    ("a_target",    "A Grade Target MW",  [r"a\s*grade"]),
    ("b_target",    "B Grade Target MW",  [r"b\s*grade"]),
    ("bel_target",  "BEL Grade Target MW", [r"\bbel\b"]),
    ("total_target", "Total Target MW",   [r"total"]),
]
...
def load_dir(dir_path: str) -> dict:
    ...
    composite labels "filename.xlsx :: SheetName" so that role-guessing can
    use filename context too (e.g. a "plan.xlsx" with a generic "Sheet1").
    """
    sheets = {}
...
def guess_sheet_roles(sheet_names: list) -> dict:
    """Guess which sheet label corresponds to which role, using both
    filename and sheet-name text (labels may be "file.xlsx :: SheetName")."""
    guesses = {}
